...
node {
    stage('Test') {
        ...
        def username='webuser'
       def info="[deploy]\n"
        ["host1", "host2", "hosts3"].each {
            h ->
            info += "${h}  ansible_user=${username} \n"
        }
        writeFile file: 'hosts', text: "$info"
...
stage('Init') {
    ...
    if (!fileExists("$HOME/bin")) {
        sh "mkdir -p ${HOME}/bin"
    }
    if (!fileExists("$HOME/bin/helm")) {
        ...
        sh "cp -f /tmp/linux-amd64/helm ${HOME}/bin; rm -rf /tmp/linux-amd64 /tmp/helm.tar.gz; chmod +x ${HOME}/bin/helm  "
    }
    if (!fileExists("$HOME/bin/kubectl")) {
        def file = "https://storage.googleapis.com/kubernetes-release/release/v1.10.3/bin/linux/amd64/kubectl"
        httpRequest ignoreSslErrors: true, outputFile: "$HOME/bin/kubectl", responseHandle: 'NONE', url: "$file"
        sh "chmod +x $HOME/bin/kubectl"
    }

    withCredentials([file(credentialsId: 'kubecerts', variable: 'config')]) {
     sh "echo $config"
    }
}
}
